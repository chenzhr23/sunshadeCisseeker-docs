...
* The Introduction page's workflow diagram and the splash logo still showed
  the pre-1.6.0 ``sun / facultative / shade`` wording (the images are
  generated by ``dev/make_images.ps1``); both were regenerated with the
  sun / shade design, and the workflow boxes now describe the current
  pipeline (Run-local in the merge, the Box 4 / G-box ratio-bin tests in
  step 08 and the portrait A4 publication figures in step 09). Every GUI
  screenshot in this documentation was regenerated from the rebuilt
  v1.6.1 interface.
