...
.. list-table::
   ...
   * - ``result/ecology_compare/07_merge/ecology_master_dataset.xlsx``
     - ``Master_long`` / ``Species_summary`` / ``Element_species`` / ``Group_species_n``; since v1.17.0 the README sheet records the ``functional_groups`` selection (``all (no selection)`` when nothing is ticked) and the master is restricted to the selected classes
   ...
   * - ``result/ecology_compare/09_figures/phylogenetic_comparative_figures.pdf`` (v1.11.0)
     - portrait A4 page: (a) in-tree species per genome type and light habitat, (b) the log10 Box4/G-box ratio density per genome type, (c) ratio per habitat with phyloANOVA P, (d) PGLS coefficient forest, (e) phylogenetic signal, (f) Box 4 vs G-box count scatter, (g) delta-AICc model comparison; every panel is drawn as pure vector ggplot2 geometry on a flat 2-column grid
   * - ``result/ecology_compare/09_figures/phylogenetic_tree_order_Box4_Gbox.pdf`` (v1.13.0, v1.14.0 time-calibrated, v1.15.0 renamed)
     - rectangular (orthogonal) phylogram per order on a rank-calibrated Mya time axis (present at the right, basal lineages at the bottom, geological periods as background bands); every labeled species appears across ~1,400-tip pages — since v1.17.0 species outside the published tree are attached at the root and drawn at the base in a grey "not in tree" block (noted in the subtitle/caption); per-clade colour blocks with an adaptive complete legend (orders below 3 tips merge into one grey bucket); each order has its OWN heat scale (within-clade percentile of log10(Box 4 / G-box), white gaps between the clade blocks) and its clade label carries the within-clade Spearman trend test of the ancestral-low → derived-high hypothesis
   * - ``result/ecology_compare/09_figures/phylogenetic_tree_family_Box4_Gbox.pdf`` (v1.13.0, v1.14.0 time-calibrated, v1.15.0 renamed)
     - the same time-calibrated rectangular tree coloured by family (families below 5 tips merge into one grey bucket)
   * - ``result/ecology_compare/09_figures/clade_trend_tests_Box4_Gbox.tsv`` (v1.14.0, v1.15.0 renamed)
     - per-clade statistics behind the Box 4 / G-box tree figures: genome type, clade level (order/family), clade, species n, Spearman rho between the species' position along the clade (basal → derived) and its log10(Box 4 / G-box), P value and direction (derived_high / derived_low / flat / no_variation)
   * - ``result/ecology_compare/09_figures/phylogenetic_tree_order_<A>_vs_<B>.pdf`` / ``phylogenetic_tree_family_<A>_vs_<B>.pdf`` (v1.15.0)
     - the same time-calibrated order/family tree figures for every unordered pair of cis-element motifs with log10(count A / count B) (ALL pairs are processed by default; both motifs must occur in >= 2 labeled species; ``SUNSHADE_MAX_MOTIF_PAIRS`` remains as an optional cap); the per-clade trend statistics are collected in ``element_pair_clade_trends.xlsx`` (one sheet per pair and level)
   * - ``result/ecology_compare/09_figures/element_pair_clade_trends.xlsx`` (v1.15.0)
     - per-clade within-clade Spearman trend tests (ancestral-low → derived-high) of log10(motif A / motif B) per genome type and clade level, one sheet per pair
   * - ``result/ecology_compare/08_statistics/clade_extremes.xlsx`` (v1.15.0)
     - per-order / per-family Box 4 / G-box extremes: each clade's maximum log10(Box 4 / G-box) = its **Box 4 expansion limit** and its minimum = its **G-box contraction limit**, with the achieving species, the clade species n, ``n_zero_box4`` (species whose ratio is 0, log10 undefined) and the median; rows sorted by decreasing max for order-vs-order / family-vs-family comparison
   * - ``result/ecology_compare/09_figures/clade_extremes.pdf`` (v1.15.0)
     - one A4 page per genome type: (a) order Box 4 expansion limits ranked, (b) order G-box contraction limits ranked, (c)/(d) the family-level versions; the 12 most extreme clades are labelled
