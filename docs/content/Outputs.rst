...
.. list-table::
   ...
   * - ``result/ecology_compare/09_figures/phylogenetic_tree_order.pdf`` (v1.13.0, v1.14.0 time-calibrated)
     - rectangular (orthogonal) phylogram per order on a rank-calibrated Mya time axis (present at the right, basal lineages at the bottom, geological periods as background bands); every in-tree species appears across ~700-tip pages; per-clade colour blocks with a complete legend (orders below 3 tips merge into one grey bucket); each order has its OWN heat scale (within-clade percentile of log10(Box 4 / G-box)) and its clade label carries the within-clade Spearman trend test of the ancestral-low → derived-high hypothesis
   * - ``result/ecology_compare/09_figures/phylogenetic_tree_family.pdf`` (v1.13.0, v1.14.0 time-calibrated)
     - the same time-calibrated rectangular tree coloured by family (families below 5 tips merge into one grey bucket)
   * - ``result/ecology_compare/09_figures/clade_trend_tests.tsv`` (v1.14.0)
     - per-clade statistics behind the tree figures: genome type, clade level (order/family), clade, species n, Spearman rho between the species' position along the clade (basal → derived) and its log10(Box 4 / G-box), P value and direction (derived_high / derived_low / flat / no_variation)
