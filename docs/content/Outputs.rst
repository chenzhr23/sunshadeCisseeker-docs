...
.. list-table::
   ...
   * - ``result/ecology_compare/09_figures/phylogenetic_tree_order_<A>_vs_<B>.pdf`` / ``phylogenetic_tree_family_<A>_vs_<B>.pdf`` (v1.15.0)
     - the same time-calibrated order/family tree figures for every unordered pair of cis-element motifs with log10(count A / count B) (both motifs present in >= 10 labeled species; the pair count is capped by ``SUNSHADE_MAX_MOTIF_PAIRS``, default 200); the per-clade trend statistics are collected in ``element_pair_clade_trends.xlsx`` (one sheet per pair and level)
